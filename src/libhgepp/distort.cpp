...
void hgeDistortionMesh::SetZ(int col, int row, float z)
{
	if (row < m_row_count && col < m_col_count)
		m_disp_array[row * m_col_count + col].z = z;
}

void hgeDistortionMesh::SetColor(int col, int row, uint32_t color)
{
	if (row < m_row_count && col < m_col_count)
		m_disp_array[row * m_col_count + col].col = color;
}
...
void hgeDistortionMesh::SetDisplacement(int col, int row, float dx, float dy, int ref)
{
	if (row < m_row_count && col < m_col_count)
	{
		...
		case HGEDISP_NODE:
			dx += col * m_cell_width;
			dy += row * m_cell_height;
			break;
		case HGEDISP_CENTER:
			dx += m_cell_width * (m_col_count - 1) / 2;
			dy += m_cell_height * (m_row_count - 1) / 2;
			break;
...
{
	if (row < m_row_count && col < m_col_count)
		return m_disp_array[row * m_col_count + col].z;
	else
		return 0.0f;
}
...
{
	if (row < m_row_count && col < m_col_count)
		return m_disp_array[row * m_col_count + col].col;
	else
		return 0;
}
...
void hgeDistortionMesh::GetDisplacement(int col, int row, float *dx, float *dy, int ref) const
{
	if (row < m_row_count && col < m_col_count)
	{
		...
		{
		case HGEDISP_NODE:
			*dx = m_disp_array[row * m_col_count + col].x - col * m_cell_width;
			*dy = m_disp_array[row * m_col_count + col].y - row * m_cell_height;
			break;

		case HGEDISP_CENTER:
			*dx = m_disp_array[row * m_col_count + col].x - m_cell_width * (m_col_count - 1) / 2;
			*dy = m_disp_array[row * m_col_count + col].x - m_cell_height * (m_row_count - 1) / 2;
			break;

		case HGEDISP_TOPLEFT:
			*dx = m_disp_array[row * m_col_count + col].x;
