...
{
	if (m_ini_file[0])
		WritePrivateProfileString(section, name, value, m_ini_file);
}

char* HGE_CALL HGE_Impl::Ini_GetString(const char *section, const char *name, const char *def_val)
{
	if (m_ini_file[0])
		GetPrivateProfileString(section, name, def_val, m_ini_string, sizeof(m_ini_string),
				m_ini_file);
	else
		strcpy(m_ini_string, def_val);
	return m_ini_string;
}
