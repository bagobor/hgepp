...
hgeFont::hgeFont(const char *szFont, bool bMipmap)
{
	...
	while(pdesc = _get_line(pdesc,linebuf))
	{
		...
		else if(!strncmp(linebuf, FNTCHARTAG, sizeof(FNTCHARTAG)-1 ))
		{
			...
			if(h>m_height) m_height=(float)h;
		}
	}
...
{
	for(int i=0; i<256; i++)
		if(m_letters[i]) delete m_letters[i];
	if(m_texture) g_hgefont_hge->Texture_Free(m_texture);
	...
	while(*string)
	{
		...
		else
		{
			...
			if(m_letters[i])
			{
				...
				fx += (m_letters[i]->GetWidth()+m_post_margin[i]+m_tracking)*m_scale*m_proportion;
			}
		}
		string++;
	}
...
float hgeFont::GetStringWidth(const char *string, bool bMultiline) const
{
	...
	while(*string)
	{
		...
		if(!bMultiline) return linew*m_scale*m_proportion;
