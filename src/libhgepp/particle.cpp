...
void hgeParticleSystem::Update(float fDeltaTime)
{
	...
	for (i = 0; i < m_alive_count; i++)
	{
		...
		if (m_update_bbox)
			m_bounding_box.Encapsulate(par->vecLocation.x, par->vecLocation.y);
	...
	if (m_age != -2.0f)
	{
		...
		{
			if (m_alive_count >= MAX_PARTICLES)
				break;

			par->fAge = 0.0f;
			par->fTerminalAge = g_hgeparticlesys_hge->Random_Float(info.fParticleLifeMin,
					info.fParticleLifeMax);

			par->vecLocation = m_prev_location + (m_location - m_prev_location)
					* g_hgeparticlesys_hge->Random_Float(0.0f, 1.0f);
			...
			ang = info.fDirection - M_PI_2 + g_hgeparticlesys_hge->Random_Float(0, info.fSpread)
					- info.fSpread / 2.0f;
			if (info.bRelative)
			...
			par->fRadialAccel = g_hgeparticlesys_hge->Random_Float(info.fRadialAccelMin,
					info.fRadialAccelMax);
			par->fTangentialAccel = g_hgeparticlesys_hge->Random_Float(info.fTangentialAccelMin,
					info.fTangentialAccelMax);

			par->fSize = g_hgeparticlesys_hge->Random_Float(info.fSizeStart, info.fSizeStart
					+ (info.fSizeEnd - info.fSizeStart) * info.fSizeVar);
			par->fSizeDelta = (info.fSizeEnd - par->fSize) / par->fTerminalAge;

			par->fSpin = g_hgeparticlesys_hge->Random_Float(info.fSpinStart, info.fSpinStart
					+ (info.fSpinEnd - info.fSpinStart) * info.fSpinVar);
			par->fSpinDelta = (info.fSpinEnd - par->fSpin) / par->fTerminalAge;

			par->colColor.r = g_hgeparticlesys_hge->Random_Float(info.colColorStart.r,
					info.colColorStart.r + (info.colColorEnd.r - info.colColorStart.r)
							* info.fColorVar);
			par->colColor.g = g_hgeparticlesys_hge->Random_Float(info.colColorStart.g,
					info.colColorStart.g + (info.colColorEnd.g - info.colColorStart.g)
							* info.fColorVar);
			par->colColor.b = g_hgeparticlesys_hge->Random_Float(info.colColorStart.b,
					info.colColorStart.b + (info.colColorEnd.b - info.colColorStart.b)
							* info.fColorVar);
			par->colColor.a = g_hgeparticlesys_hge->Random_Float(info.colColorStart.a,
					info.colColorStart.a + (info.colColorEnd.a - info.colColorStart.a)
							* info.fAlphaVar);
...
void hgeParticleSystem::Render()
{
	...
	for (i = 0; i < m_alive_count; i++)
	{
		...
		info.sprite->RenderEx(par->vecLocation.x * m_scale + m_tx, par->vecLocation.y * m_scale
				+ m_ty, par->fSpin * par->fAge, par->fSize * m_scale);
		par++;
	}
