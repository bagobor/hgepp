...
void HGE_CALL HGE_Impl::Gfx_SetClipping(int x, int y, int w, int h)
{
	...
	D3DXMatrixOrthoOffCenterLH(&tmp, (float) vp.X, (float) (vp.X + vp.Width), -((float) (vp.Y
			+ vp.Height)), -((float) vp.Y), vp.MinZ, vp.MaxZ);
	D3DXMatrixMultiply(&m_proj_matrix, &m_proj_matrix, &tmp);
...
bool HGE_CALL HGE_Impl::Gfx_BeginScene(HTARGET targ)
{
	...
	else if (hr == D3DERR_DEVICENOTRESET)
	{
		if (m_windowed)
		{
			if (FAILED(m_d3d->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &Mode)) || Mode.Format
					== D3DFMT_UNKNOWN)
			{
		...
		float z)
{
	if (m_vertices)
	{
		if (m_cur_prim_type != HGEPRIM_LINES || m_prim_count >= VERTEX_BUFFER_SIZE / HGEPRIM_LINES
				|| m_cur_texture || m_cur_blend_mode != BLEND_DEFAULT)
		{
			_render_batch();
		...
		m_vertices[i].tx = m_vertices[i + 1].tx = m_vertices[i].ty = m_vertices[i + 1].ty = 0.0f;
...
void HGE_CALL HGE_Impl::Gfx_RenderTriple(const hgeTriple *triple)
{
	if (m_vertices)
	{
		if (m_cur_prim_type != HGEPRIM_TRIPLES || m_prim_count >= VERTEX_BUFFER_SIZE
				/ HGEPRIM_TRIPLES || m_cur_texture != triple->tex || m_cur_blend_mode
				!= triple->blend)
		{
			_render_batch();
		...
		memcpy(&m_vertices[m_prim_count * HGEPRIM_TRIPLES], triple->v, sizeof(hgeVertex)
				* HGEPRIM_TRIPLES);
		m_prim_count++;
	}
}
...
void HGE_CALL HGE_Impl::Gfx_RenderQuad(const hgeQuad *quad)
{
	if (m_vertices)
	{
		if (m_cur_prim_type != HGEPRIM_QUADS || m_prim_count >= VERTEX_BUFFER_SIZE / HGEPRIM_QUADS
				|| m_cur_texture != quad->tex || m_cur_blend_mode != quad->blend)
		{
			_render_batch();
...
hgeVertex* HGE_CALL HGE_Impl::Gfx_StartBatch(primitive_mode_t prim_type, HTEXTURE tex, int blend,
		int *max_prim)
{
	if (m_vertices)
	{
...
bool HGE_Impl::_GfxInit()
{
	...
	System_Log("Version: %d.%d.%d.%d", HIWORD(AdID.DriverVersion.HighPart),
			LOWORD(AdID.DriverVersion.HighPart), HIWORD(AdID.DriverVersion.LowPart),
	...
	for (i = 0; i < nModes; i++)
	{
		...
		if (Mode.Width != (UINT) m_scr_width || Mode.Height != (UINT) m_scr_height)
			continue;
		if (m_color_depth == 16 && (_format_id(Mode.Format) > _format_id(D3DFMT_A1R5G5B5)))
			continue;
	...
	uint32_t vp;
	if ((caps.VertexShaderVersion < D3DVS_VERSION(1, 1)) || !(caps.DevCaps
...
void HGE_Impl::_AdjustWindow()
{
	...
	if (m_windowed)
	{
		...
		SetWindowPos(m_hwnd, HWND_NOTOPMOST, rc->left, rc->top, rc->right - rc->left, rc->bottom
				- rc->top, SWP_FRAMECHANGED);
	}
	else
	{
		SetWindowLong(m_hwnd, GWL_EXSTYLE, style | WS_EX_TOPMOST);
		SetWindowPos(m_hwnd, HWND_TOPMOST, rc->left, rc->top, rc->right - rc->left, rc->bottom
				- rc->top, SWP_FRAMECHANGED);
	}
}
...
bool HGE_Impl::_GfxRestore()
{
	...
	if (m_gfxrestore_func)
		return m_gfxrestore_func();
